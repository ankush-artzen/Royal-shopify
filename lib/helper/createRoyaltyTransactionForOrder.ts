...
export async function createRoyaltyTransactionForOrder({
  ...
  // 1️⃣ Check if transaction already exists using the CORRECT unique constraint
  const existingTx = await prisma.royaltyTransaction.findUnique({
    where: {
      shop_orderId_productId_designerId: {
        ...
        orderId,
        productId,
        designerId: designerId || "", // Handle case where designerId might be undefined
      },
    },
  });
  ...
  if (existingTx) {
    console.log(
      `⚠️ Transaction already exists for order ${orderId}, product ${productId}, designer ${designerId}. Skipping Shopify charge.`,
    );
    return existingTx;
  }
  ...
  // 3️⃣ Create new transaction record
  const royaltyTransaction = await prisma.royaltyTransaction.create({
    data: {
      ...
      royaltypercentage,
      designerId: designerId || null, // Handle optional designerId
